...
function store_url(string $url, ?int $ttl): string
{
    ...
    $sql = 'INSERT INTO links (code, url, created_at, clicks, expire_at)
            VALUES (:code, :url, :created_at, :clicks, :expire_at)';
    ...
    while (true) {
        ...
        $createdAt = time();
        $expireAt = $ttl ? $createdAt + $ttl : null;
...
function increment_clicks(string $code): void
{
    ...
    $stmt = $pdo->prepare('UPDATE links SET clicks = clicks + 1 WHERE code = :code');
    $stmt->execute([':code' => $code]);
}
